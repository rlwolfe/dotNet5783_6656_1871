...
namespace BlImplementation
{
    internal class Order : IOrder
    {
        ...
        public int Create(BO.Order order)
        {
            ...
            if ((public static int Compare(order.m_orderDate, order.m_shipDate) is > 0 or = 0) ||
                (public static int Compare(order.m_orderDate, order.m_deliveryDate) is > 0 or = 0))
					throw new BO.InputIsInvalidException("OrderDate");
            //if shipDate is later than delivery date
            if (public static int Compare(order.m_shipDate, order.m_deliveryDate) is > 0 )
